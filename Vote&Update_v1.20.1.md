# Lumera v1.20.1 Güncelleme Talimatları
...
lumerad tx gov vote 14 yes --from wallet --chain-id lumera-testnet-2 --gas-prices 0.1ulume --gas auto --gas-adjustment 1.6 -y
```
...
### Adım 3: v1.20.1 Binary'sini İndir 📥

```bash
wget -O lumera_v1.20.1_linux_amd64.tar.gz https://github.com/LumeraProtocol/lumera/releases/download/v1.20.1/lumera_v1.20.1_linux_amd64.tar.gz
```

### Adım 4: Arşivi Çıkar 📦

```bash
tar -xvzf lumera_v1.20.1_linux_amd64.tar.gz
```
...
```bash
mkdir -p $HOME/.lumera/cosmovisor/upgrades/v1.20.1/bin
```

### Adım 7: Binary'yi Upgrade Dizinine Taşı 🚀

```bash
mv $HOME/lumerad $HOME/.lumera/cosmovisor/upgrades/v1.20.1/bin/lumerad
```

### Adım 8: Kurulumu Kontrol Et ✔️

```bash
ls -l $HOME/.lumera/cosmovisor/upgrades/v1.20.1/bin/
```
...
# Yeni versiyonu kontrol et (v1.20.1 olmalı)
lumerad version
...
rm -f $HOME/lumera_v1.20.1_linux_amd64.tar.gz
```
