# Lumera v1.20.0 Güncelleme Talimatları
...
lumerad tx gov vote 11 yes --from wallet --chain-id lumera-testnet-2 --gas-prices 0.1ulume --gas auto --gas-adjustment 1.6 -y
```
...
### Adım 3: v1.20.0 Binary'sini İndir 📥

```bash
wget -O lumera_v1.20.0_linux_amd64.tar.gz https://github.com/LumeraProtocol/lumera/releases/download/v1.20.0/lumera_v1.20.0_linux_amd64.tar.gz
```

### Adım 4: Arşivi Çıkar 📦

```bash
tar -xvzf lumera_v1.20.0_linux_amd64.tar.gz
```
...
```bash
mv $HOME/lumerad $HOME/.lumera/cosmovisor/upgrades/v1.20.0/bin/lumerad
```

### Adım 8: Kurulumu Kontrol Et ✔️

```bash
ls -l $HOME/.lumera/cosmovisor/upgrades/v1.20.0/bin/
```
...
# Yeni versiyonu kontrol et (v1.12.0 olmalı)
lumerad version
...
rm -f $HOME/lumera_v1.20.0_linux_amd64.tar.gz
```
