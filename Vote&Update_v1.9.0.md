# Lumera v1.9.0 Güncelleme Talimatları

## 📋 Güncelleme Adımları

### Adım 1: Governance Oylaması 🗳️
```bash
# Proposal #7 için OY VER
lumerad tx gov vote 7 yes --from wallet --chain-id lumera-testnet-2 --gas-prices 0.1ulume --gas auto --gas-adjustment 1.6 -y
```
...
### Adım 3: v1.9.0 Binary'sini İndir 📥
```bash
wget -O lumera_v1.9.0_linux_amd64.tar.gz https://github.com/LumeraProtocol/lumera/releases/download/v1.9.0/lumera_v1.9.0_linux_amd64.tar.gz
```

### Adım 4: Arşivi Çıkar 📦
```bash
tar -xvzf lumera_v1.9.0_linux_amd64.tar.gz
```
...
```bash
mv $HOME/lumerad $HOME/.lumera/cosmovisor/upgrades/v1.9.0/bin/lumerad
```

### Adım 8: Kurulumu Kontrol Et ✔️
```bash
ls -l $HOME/.lumera/cosmovisor/upgrades/v1.9.0/bin/
```
